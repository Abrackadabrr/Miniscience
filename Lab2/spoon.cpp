...
public:
    ...
    void setVel(double _vx, double _vy, double _vz){ vx = _vx; vy = _vy; vz = _vz; };
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        nodes.resize(nodesCoords.size() / 3);  // делим на три так как там лежат три координамы подряд для каждого узла
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            if (pointY < min_y) min_y = pointY;
            // Модельная скалярная величина распределена как-то вот так
            double smth = pow(pointX, 4) + pow(pointY, 6) + pow(pointZ, 7);
    ...
    void doTimeStep(double tau) {
        ...
        for(auto & node : nodes) {
            if ((omega/k)*time > node.y - min_y)
                node.setVel(0.0, 0.0, 0.5*omega*std::cos(omega*time - k*node.y));
            node.move(tau);
        }
    }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "spoon-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    gmsh::initialize();
    gmsh::model::add("spoon");

    try {
        gmsh::merge("../Coffee_Spoon.stl");
    } catch(...) {
    ...
    gmsh::finalize();
    for (int i = 0; i < 150; i++) {mesh.snapshot(i); mesh.doTimeStep(0.07);}
